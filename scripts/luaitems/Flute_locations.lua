...
function Flute_locations_scope(scope_direction, scope_location_obj, scope_side)
    ...
    function _SetFluteLocationOptions(source, target) -- source == inside, target == outside
    ...
    ---@param source LuaItem
    function _UnsetFluteLocationOptions(source)
        if MANUAL_CHECKED and er_storage_item then
            -- local er_storage_item = (Tracker:FindObjectForCode("manual_er_storage") --[[@as LuaItem]]).ItemState
    ...
    local function _LeftClickUnmarkHelper(target, source)
        ...
        if target_entrance_from ~= nil then
            _UnsetFluteLocationOptions(target_entrance_from)
        end
        if target_entrance_to ~= nil then
            _UnsetFluteLocationOptions(target_entrance_to)
        end
        _UnsetFluteLocationOptions(source_entrance_to)
        _UnsetFluteLocationOptions(source_entrance_from)
    end
    ...
    local function _LeftClickMarkHelper(target, source)
        ...
        local source_entrance_to = NAMED_ER_CONNECTIONS["to_" .. source] --[[@as LuaItem]]
        if target_entrance_from ~= nil then
            _SetFluteLocationOptions(source_entrance_from, target_entrance_to) -- enter source exit target
            _SetFluteLocationOptions(target_entrance_to, source_entrance_from)
        end 
        if target_entrance_from and target_entrance_to ~= nil then
            _SetFluteLocationOptions(target_entrance_from, source_entrance_to) -- enter target exit source
            _SetFluteLocationOptions(source_entrance_to, target_entrance_from)
        end
    end
...
--         _UnsetFluteLocationOptions(NAMED_ER_CONNECTIONS[name]--[[@as LuaItem]])
--     end
