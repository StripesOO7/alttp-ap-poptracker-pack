Tracker:AddLayouts("layouts/events.json")
Tracker:AddLayouts("layouts/settings.json")
Tracker:AddLayouts("layouts/settings_tab_core.json")
-- Tracker:AddLayouts("layouts/settings_tab_beta.json")
-- Tracker:AddLayouts("layouts/settings_tab_alttpr.json")
Tracker:AddLayouts("layouts/popup.json")
Tracker:AddLayouts("layouts/items.json")
-- Tracker:AddLayouts("layouts/tabs_er_enemy_pots.json")
-- Tracker:AddLayouts("layouts/tabs_enemy_pots.json")
-- Tracker:AddLayouts("layouts/tabs_er.json")
Tracker:AddLayouts("layouts/tabs.json")
Tracker:AddLayouts("layouts/flute_maps.json")
Tracker:AddLayouts("layouts/layouts.json")
Tracker:AddLayouts("layouts/tracker.json")
Tracker:AddLayouts("layouts/broadcast.json")
Tracker:AddLayouts("layouts/dungeon_items.json")
-- Tracker:AddLayouts("layouts/dungeon_items_keydrop.json")
-- Tracker:AddLayouts("layouts/entrances_reduced.json")
Tracker:AddLayouts("layouts/entrances.json")
Tracker:AddLayouts("layouts/dungeons_pots_enemies.json")
Tracker:AddLayouts("layouts/dungeons.json")
Tracker:AddLayouts("layouts/dungeons_doors.json")
Tracker:AddLayouts("layouts/route.json")
Tracker:AddLayouts("layouts/dmg_table.json")
Tracker:AddLayouts("layouts/dmg_table_bodies.json")
Tracker:AddLayouts("layouts/dmg_table_heads.json")