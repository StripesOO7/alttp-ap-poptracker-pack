...
gt_bottom_main_room:connect_two_ways(gt_bottom_hope_room, function(keys) return has("gt_smallkey", keys + 1, 4, keys + 2, 6), KDSreturn(keys + 1, keys + 1) end)
...
gt_bottom_bonk_pit_room:connect_two_ways(gt_bottom_map_room, function(keys) 
    return all(
        has("hookshot"),
        has("gt_smallkey", keys, 4, keys + 1, 8)
    ), KDSreturn(keys, keys + 1)
end)
gt_bottom_bonk_pit_room:connect_two_ways(gt_bottom_double_switch_room, function(keys) 
    return all(
        has("hookshot"), 
        has("gt_smallkey", keys, 3, keys + 2, 6)
    ) 
end)
...
gt_bottom_double_switch_room:connect_two_ways(gt_bottom_firesnake_room, function(keys) return has("gt_smallkey", keys, 3, keys + 3, 7), KDSreturn(keys, keys + 1) end)
...
gt_bottom_firesnake_room:connect_two_ways(gt_bottom_teleporter_puzzle_room, function(keys) return has("gt_smallkey", keys + 1, 4, keys + 4, 8), KDSreturn(keys + 1, keys + 1) end)
...
gt_bottom_tile_room:connect_two_ways(gt_bottom_torch_puzzle, function(keys) return has("gt_smallkey", keys + 1, 3, keys + 2, 7), KDSreturn(keys + 1, keys + 1) end)
...
gt_bottom_conveyor_star_room:connect_one_way(gt_bottom_invisibile_bonk_room, function(keys) return has("gt_smallkey", keys + 1, 4, keys + 4, 7), KDSreturn(keys + 1, keys + 1) end)
...
gt_top_mini_helmasaur_room:connect_two_ways(gt_top_pre_moldorm_room, function(keys) 
    return all(
        has("gt_smallkey", keys, 3, keys + 1, 7),
        has("bombs")
    ), KDSreturn(keys, keys + 1)
end)
...
gt_top_pre_moldorm_room:connect_one_way(gt_top_top_refight, function(keys) return has("gt_smallkey", keys + 1, 4, keys + 1, 8), KDSreturn(keys + 1, keys + 1) end)
