--lightworld

light_spawn_links_house_area = alttp_location.new()
light_spawn_sanctuary = alttp_location.new()
light_spawn_old_man = alttp_location.new()
kakariko_village = alttp_location.new()
south_of_village = alttp_location.new()
sanctuary_area = alttp_location.new()
lost_woods = alttp_location.new()
dam_area = alttp_location.new()
desert_area = alttp_location.new()
lumberjacks_area = alttp_location.new()
light_lake_hylia = alttp_location.new()
links_house_area = alttp_location.new()
eastern_palace_area = alttp_location.new()
zora_river = alttp_location.new()
hyrule_castle_area = alttp_location.new()
witchhut = alttp_location.new()
light_death_mountain_left_bottom = alttp_location.new()
light_death_mountain_left_top = alttp_location.new()
light_death_mountain_right_bottom = alttp_location.new()
light_death_mountain_right_top = alttp_location.new()
hyrule_castle = alttp_location.new()
hyrule_castle_top_outside = alttp_location.new()
eastern_palace = alttp_location.new()
desert_palace = alttp_location.new()
tower_of_hera = alttp_location.new()
agahnims_tower = alttp_location.new()
hyrule_castle_front = alttp_location.new()
hyrule_castle_top_left = alttp_location.new()
hyrule_castle_top_right = alttp_location.new()
desert_palace_front = alttp_location.new()
desert_palace_left = alttp_location.new()
desert_palace_right = alttp_location.new()
desert_palace_back = alttp_location.new()
checkerboard_lege = alttp_location.new()
light_flute_map = alttp_location.new()
lightworld_spawns = alttp_location.new()
kakariko_well = alttp_location.new()
kakariko_well_cave = alttp_location.new()
kakariko_well_hole = alttp_location.new()
kakariko_well_item = alttp_location.new()
kakariko_house_top_left = alttp_location.new()
kakariko_blinds_hideout = alttp_location.new()
kakariko_house_hill_top = alttp_location.new()
kakariko_house_right_top = alttp_location.new()
kakariko_chickenhut = alttp_location.new()
kakariko_sick_kid = alttp_location.new()
kakariko_house_right_center = alttp_location.new()
kakariko_bombhut = alttp_location.new()
kakariko_shop = alttp_location.new()
kakariko_backside_pub = alttp_location.new()
kakariko_frontside_pub = alttp_location.new()
darf_smiths = alttp_location.new()
magic_bat_hole = alttp_location.new()
magic_bat_item = alttp_location.new()
magic_bat_cave = alttp_location.new()
light_activate_flute = alttp_location.new()
inverted_activate_flute = alttp_location.new()
library = alttp_location.new()
shooting_gallery = alttp_location.new()
twin_house = alttp_location.new()
twin_house_left = alttp_location.new()
twin_house_right = alttp_location.new()
race_ledge = alttp_location.new()
kings_tomb = alttp_location.new()
kings_tomb_inside = alttp_location.new()
graveyard_ledge_cave = alttp_location.new()
graveyard_ledge_inside = alttp_location.new()
graveyard_fairy = alttp_location.new()
sanctuary = alttp_location.new()
archery_minigame = alttp_location.new()
lost_woods_fortune = alttp_location.new()
lost_woods_hideout_cave = alttp_location.new()
lost_woods_hideout_hole = alttp_location.new()
lost_woods_hideout_item = alttp_location.new()
lost_woods_top = alttp_location.new()
lost_woods_pedestal = alttp_location.new()
dam_top_right_cave = alttp_location.new()
dam_inside = alttp_location.new()
sunken_treasure = alttp_location.new()
mini_moldorm_cave = alttp_location.new()
dam_desert_fairy = alttp_location.new()
twenty_rupee_thief = alttp_location.new()
aginah_cave = alttp_location.new()
checkerboard_cave = alttp_location.new()
desert_ledge = alttp_location.new()
lumberjacks_hole = alttp_location.new()
lumberjacks_cave = alttp_location.new()
lumberjacks_item = alttp_location.new()
lumberjacks_house = alttp_location.new()
death_mountain_accend = alttp_location.new()
light_bumper_cave = alttp_location.new()
light_bumper_cave_ledge = alttp_location.new()
sanctuary_bonk_cave = alttp_location.new()
light_lake_fortune = alttp_location.new()
light_lake_shop = alttp_location.new()
upgrade_fairy = alttp_location.new()
hobo = alttp_location.new()
icerod_cave = alttp_location.new()
icerod_cave_inside = alttp_location.new()
icerod_fairy = alttp_location.new()
icerod_stone = alttp_location.new()
links_fairy_fountain = alttp_location.new()
links_house = alttp_location.new()
cave45 = alttp_location.new()
cave45_ledge = alttp_location.new()
eastern_teleporter_cave = alttp_location.new()
eastern_fairy = alttp_location.new()
sahasralahs_hut = alttp_location.new()
sahasralahs_hut_back = alttp_location.new()
waterfall_fairy = alttp_location.new()
waterfall_fairy_inside = alttp_location.new()
secret_passage_hole = alttp_location.new()
secret_passage_stairs = alttp_location.new()
secret_passage = alttp_location.new()
light_potion_shop = alttp_location.new()
darkworld_teleport_kakariko_village = alttp_location.new()
darkworld_teleport_turtle_rock = alttp_location.new()
darkworld_teleport_desert_area = alttp_location.new()
lightworld_overworld = alttp_location.new()
light_death_mountain = alttp_location.new()
spectacle_rock_cave = alttp_location.new()
ether_tablet = alttp_location.new()
bombos_tablet = alttp_location.new()
purple_chest_return = alttp_location.new()
lake_hylia_island = alttp_location.new()
spectacle_rock_top = alttp_location.new()
spectacle_rock_insdie_bottom = alttp_location.new()
spectacle_rock_insdie_top = alttp_location.new()
spiral_cave_top = alttp_location.new()
spiral_cave_bottom = alttp_location.new()
mimic_cave = alttp_location.new()
baradox_cave_top = alttp_location.new()
baradox_cave_bottom = alttp_location.new()
baradox_cave_inside_top = alttp_location.new()
baradox_cave_inside_bottom = alttp_location.new()
turtle_rock_pedestal = alttp_location.new()
light_death_mountain_shop = alttp_location.new()
light_eyebridge_fairy = alttp_location.new()
light_death_mountain_old_man_cave = alttp_location.new()
light_death_mountain_cave1 = alttp_location.new()
light_death_mountain_cave2 = alttp_location.new()
light_death_mountain_cave3 = alttp_location.new()
light_death_mountain_ascend = alttp_location.new()



--darkwourld

darkworld_spawns = alttp_location.new()
big_bomb_shop_area = alttp_location.new()
swamp_area = alttp_location.new()
mire_area = alttp_location.new()
dark_lake_hylia = alttp_location.new()
dark_icerod_area = alttp_location.new()
village_of_the_outcast = alttp_location.new()
south_of_village_of_the_outcast = alttp_location.new()
skull_woods_area = alttp_location.new()
dark_lumpberjacks = alttp_location.new()
dark_chapel_area = alttp_location.new()
dark_chapel = alttp_location.new()
dark_potion_shop = alttp_location.new()
catfish_area = alttp_location.new()
pyramid = alttp_location.new()
pod_area = alttp_location.new()

dark_death_mountain_left_top = alttp_location.new()
dark_death_mountain_right_top = alttp_location.new()
dark_death_mountain_left_bottom = alttp_location.new()
dark_death_mountain_right_bottom = alttp_location.new()

palace_of_darkness = alttp_location.new()
swamp_palace = alttp_location.new()
skull_woods = alttp_location.new()
skull_woods_gibdo_entrance = alttp_location.new()
skull_woods_big_chest_entrance = alttp_location.new()
skull_woods_pot_circle_drop = alttp_location.new()
skull_woods_bottom_left_drop = alttp_location.new()
skull_woods_pinball_drop = alttp_location.new()
skull_woods_north_drop = alttp_location.new()
skull_woods_west_lobby_entrance = alttp_location.new()
skull_woods_back_entrance = alttp_location.new()
thieves_town = alttp_location.new()
ice_palace = alttp_location.new()
misery_mire = alttp_location.new()
turtle_rock = alttp_location.new()
ganons_tower = alttp_location.new()
dark_flute_map = alttp_location.new()
dark_spawn_links_house = alttp_location.new()
dark_spawn_dark_chapel = alttp_location.new()
dark_spawn_old_man = alttp_location.new()
big_bomb_shop = alttp_location.new()
big_bomb_shop_fairy_cave = alttp_location.new()
hype_cave = alttp_location.new()
hype_cave_back = alttp_location.new()
mire_shed_left = alttp_location.new()
mire_shed_right = alttp_location.new()
dark_aghina = alttp_location.new()
dark_lake_shop = alttp_location.new()
dark_icerod_cave = alttp_location.new()
dark_icerod_fairy = alttp_location.new()
dark_icerod_stone = alttp_location.new()
chest_game = alttp_location.new()
c_shaped_house = alttp_location.new()
dark_village_shop = alttp_location.new()
brewery = alttp_location.new()
purple_chest_pickup = alttp_location.new()
dwarf_pickup = alttp_location.new()
peg_cave = alttp_location.new()
peg_cave_inside = alttp_location.new()
dark_archery_minigame = alttp_location.new()
dark_lumpberjacks_shop = alttp_location.new()
red_shield_shop = alttp_location.new()
dark_death_mountain_decent = alttp_location.new()
bumber_cave = alttp_location.new()
dark_chapel = alttp_location.new()
dark_potion_shop_inside = alttp_location.new()
pyramid_inside = alttp_location.new()
pyramid_fairy_cave = alttp_location.new()
pod_teleport_cave = alttp_location.new()
pod_fairy_cave = alttp_location.new()
kiki_cave = alttp_location.new()
stumpy = alttp_location.new()
hookshot_cave = alttp_location.new()
floating_island = alttp_location.new()
superbunny_cave = alttp_location.new()
spike_cave = alttp_location.new()
dark_death_mountain_shop = alttp_location.new()